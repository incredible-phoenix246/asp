      ...
      <div className="flex top-0 relative z-10 text-white h-full items-center w-full sm:bg-black/50 lg:bg-black/70 justify-center px-2 sm:px-4 lg:px-8 transition-colors duration-500">
        <div className="flex flex-col gap-y-5 w-full sm:max-w-[90%]   xl:max-w-[85%] 2xl:max-w-[90%]  items-center">
          ...
          <p className="w-full sm:max-w-[80%] text-center sm:font-medium text-white/80 text-sm sm:text-lg">
            Aspace Network and Technology Limited specializes in creating
            innovative and scalable technology solutions for academic
            institutions. Our team of experts works together to address the
            unique challenges faced by schools, transforming the way they
            operate.
          </p>

          <div className="mt-3 lg:mt-6 w-full justify-center flex gap-x-5 lg:gap-x-10 [&>button]:border-white [&>button]:border [&>button]:px-4 [&>button]:lg:px-12 [&>button]:py-2 [&>button]:lg:py-4 [&>button]:rounded-md [&>button:last-child]:bg-white [&>button:last-child]:text-brand-blue [&>button]:font-medium [&>button]:lg:font-semibold [&>button]:text-white [&>button]:max-[390px]:text-sm max-[350px]:flex-col gap-y-3">
            <button type="button">
